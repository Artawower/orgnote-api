...
describe('agenda parity — sample use cases', () => {
  test('completeTask: TODO → DONE with CLOSED + LOGBOOK state change', () => {
  ...
  test('completeRepeatingTask: keyword stays, scheduled advances, LOGBOOK records', () => {
  ...
  test('reopenTask: DONE → TODO with CLOSED removed and LOGBOOK recorded', () => {
    const next = editOrgDocument(
  ...
  test('undoRecurringCompletion: removes a single LOGBOOK state-change line', () => {
    const next = editOrgDocument(
  ...
  test('completeHabit: keyword reset, scheduled advanced, CLOCK appended', () => {
    const next = editOrgDocument(
      '* TODO Meditate\nSCHEDULED: <2026-05-13 Wed +1d>\n',
      (doc) => {
        const h = doc.headlineAt(0);
  ...
  test('openClock + closeClock cycle', () => {
  ...
  test('changeTaskTitle / changeTaskStatus / changeTaskBody', () => {
